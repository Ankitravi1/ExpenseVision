...
- **Remaining:** run `npx prisma db push` (blocked during build by the running dev server locking the query engine) to create the table/column; port the Settings "Which AI to use" toggle to **mobile**; optional **usage quotas** (host pays for platform-key usage — add per-user monthly caps).
